...
5. Choose a voice model and adjust voice volume or speech settings if needed.
...
- **Voice volume**: Server-side volume for Piper TTS callouts before browser player output. This does not affect the static audio check.
...
- **Play audio check**: Plays a bundled music clip through the Sendspin path. This is not affected by **Voice volume**.
...
- **Voice volume** is applied at playback time; changing it does not require a cache rebuild.

## Operational Notes

- Race Voice does not disable RotorHazard's built-in browser speech. Set Voice Volume to `0` on regular RotorHazard browser clients to avoid duplicate callouts.
...
- Callouts are generated server-side; browser-specific RotorHazard voice settings do not affect Race Voice output. Use the plugin **Voice volume** for global TTS level and the `/player` volume for per-device output level.
